...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 添加应急按钮，在常规事件卡住时可以跳转到下一个状态
    setTimeout(() => {
        const emergencyButton = document.createElement('button');
        emergencyButton.textContent = '应急：跳过当前事件';
        ...
        emergencyButton.style.backgroundColor = '#3498db';
        emergencyButton.style.color = 'white';
        emergencyButton.style.border = 'none';
        emergencyButton.style.padding = '10px';
        emergencyButton.style.cursor = 'pointer';
        emergencyButton.style.display = 'none';
        emergencyButton.id = 'emergency-regular-button';
        
        // 当检测到常规事件时显示此按钮
        const checkForRegularEvent = setInterval(() => {
            if (window.currentGame && window.currentGame.currentEvent && 
                (window.currentGame.currentEvent.type === 'morning' || 
                 window.currentGame.currentEvent.type === 'home' || 
                 window.currentGame.currentEvent.type === 'phone')) {
        ...
        emergencyButton.addEventListener('click', () => {
            if (window.currentGame && window.currentGame.regularHandler) {
                console.log('[应急] 触发应急跳过常规事件');
                
                const handler = window.currentGame.regularHandler;
                
                // 根据当前事件类型处理
                if (handler.currentEventType === 'morning') {
                    // 判断当前是否是特殊天数（只有一个事件的天数）
                    const isSpecialDay = handler.specialDays.includes(window.currentGame.gameState.day);
                    
                    if (isSpecialDay || handler.secondEventLoaded) {
                        // 特殊天数或已加载第二个事件，直接开始新的一天
                        console.log('[应急] 直接开始新的一天');
                        window.currentGame.startNewDay();
                    } else {
                        // 尝试加载第二个事件
                        console.log('[应急] 尝试加载第二个事件');
                        handler.loadSecondEvent();
                    }
                } else {
                    // 非晨间事件，直接开始新的一天
                    console.log('[应急] 直接开始新的一天');
