...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        emergencyButton.addEventListener('click', () => {
            if (window.currentGame) {
                console.log('[修复] 触发应急跳过周末事件');
                
                // 根据当前状态处理
                if (window.currentGame.hasCompletedQuiz && window.currentGame.isProcessingBranchDialogues) {
                    // 如果在分支对话中，直接结束事件
                    console.log('[修复] 应急按钮：跳过分支对话');
                    ...
                    // 已完成答题但可能卡在其他地方
                    console.log('[修复] 应急按钮：已完成答题，直接结束事件');
                    window.currentGame.hasProcessedBranches = true; // 设置已处理标记
                    window.currentGame.endEvent();
                } else {
                    // 尚未完成答题，设置为完成状态
                    console.log('[修复] 应急按钮：设置答题为完成状态');
                    window.currentGame.hasCompletedQuiz = true;
                    window.currentGame.quizScore = 70; // 设置一个默认及格分数
                    window.currentGame.endEvent();
                }
            }
        });
