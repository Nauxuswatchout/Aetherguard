...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 添加应急按钮，在周末事件完全卡住时可以跳转到下一个状态
    setTimeout(() => {
        const emergencyButton = document.createElement('button');
        emergencyButton.textContent = '应急：跳过当前周末事件';
        emergencyButton.style.position = 'fixed';
        emergencyButton.style.right = '20px';
        emergencyButton.style.bottom = '80px';
        ...
        emergencyButton.id = 'emergency-weekend-button';
        
        // 当检测到周末事件时显示此按钮
        const checkForWeekendEvent = setInterval(() => {
        ...
        emergencyButton.addEventListener('click', () => {
            if (window.currentGame && window.currentGame.weekendHandler) {
                console.log('[应急] 触发应急跳过周末事件');
                
                const handler = window.currentGame.weekendHandler;
                
                // 根据当前状态处理
                if (handler.hasCompletedQuiz && handler.isProcessingBranchDialogues) {
                    // 如果在分支对话中，直接结束事件
                    console.log('[应急] 应急按钮：跳过分支对话');
                    handler.isProcessingBranchDialogues = false;
                    handler.hasProcessedBranches = true;
                    handler.endEvent();
                } else if (handler.hasCompletedQuiz) {
                    // 已完成答题但可能卡在其他地方
                    console.log('[应急] 应急按钮：已完成答题，直接结束事件');
                    handler.hasProcessedBranches = true;
                    handler.endEvent();
                } else {
                    // 尚未完成答题，设置为完成状态
                    console.log('[应急] 应急按钮：设置答题为完成状态');
                    handler.hasCompletedQuiz = true;
                    handler.quizScore = 70; // 设置一个默认及格分数
                    handler.endEvent();
                }
            }
        });
