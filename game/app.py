...
def get_event(event_type):
    ...
    # 输出每种事件文件的存在情况
    for evt_type, paths in {'morning': ['js/morning-events.json'], 
                           'home': ['js/home-events.json'], 
                           'phone': ['js/phone-events.json'], 
                           'weekend': ['js/weekend-events.json', 'js/weekend-events-2.json']}.items():
        for path in paths:
            full_path = os.path.join(base_dir, path)
            print(f"Event file {evt_type}: {full_path} (exists: {os.path.exists(full_path)})")
    ...
    # 输出已加载的事件数量
    for evt_type, evt_list in events.items():
        print(f"事件类型 {evt_type}: 加载了 {len(evt_list)} 个事件")
    ...
    for event in events[event_type]:
        ...
        if is_event_available(event, game_state):
            ...
            print(f"事件 {event['id']} 天数条件匹配: {game_state['day']}" if has_day_condition else f"事件 {event['id']} 可用")
    
    # 检查是否有可用事件
    if available_events:
        # 记录找到的可用事件数量
        print(f"找到 {len(available_events)} 个匹配当前天数的事件")
        ...
        if day_matching_events:
            ...
            print(f"事件 {selected_event['id']} 符合当天日期条件，优先返回")
        else:
            # 没有匹配的日期，选择第一个可用事件
        ...
        print(f"返回匹配当天的事件: {selected_event['id']}")
...
@app.route('/new_game_main')
def new_game_main():
    """全新开始游戏，完全重置状态"""
    print("完全重置游戏状态，开始新游戏")
    session['game_state'] = init_game_state()
    
    # 添加时间戳，确保浏览器不使用缓存版本
    timestamp = int(time.time())
    return render_template('game.html', reset_cache=timestamp)
